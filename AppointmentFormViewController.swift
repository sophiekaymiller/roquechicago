...
class AppointmentNavController: UINavigationController, RowControllerType {
	var onDismissCallback : ((UIViewController) -> ())?
}
class AppointmentFormViewController: FormViewController {
	...
	private func initializeForm() {
		...
		form +++
			TextRow("Appointment").cellSetup { cell, row in
				cell.textField.placeholder = row.tag
			}
			
			<<< TextRow("Salon").cellSetup {
	...
	@objc func cancelTapped(_ barButtonItem: UIBarButtonItem) {
		(navigationController as? AppointmentNavController)?.onDismissCallback?(self)
	}
